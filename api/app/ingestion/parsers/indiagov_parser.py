...
def parse_indiagov_date(raw_text: str) -> Optional[datetime]:
    ...
    clean = re.sub(r"^(?:Last Updated|Updated|Date|अंतिम अद्यतन)\s*[:\-]?\s*", "", clean, flags=re.I).strip()
...
class IndiaGovParser(BaseParser):
    ...
    def parse(self, response: FetchResponse) -> ParsedContent:
        ...
        published_at = None
        date_patterns = [
            r"(?:Last Updated|Updated|Date|अंतिम अद्यतन)\s*[:\-]?\s*([A-Za-z]{3,9}\s+\d{1,2}[,\s]+\d{4})",
            r"(?:Last Updated|Updated|Date)\s*[:\-]?\s*(\d{1,2}\s+[A-Za-z]{3,9}[,\s]+\d{4})",
            r"(\d{1,2}[/.-]\d{1,2}[/.-]\d{4})",
        ]
