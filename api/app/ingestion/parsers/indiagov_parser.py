...
def parse_indiagov_date(raw_text: str) -> Optional[datetime]:
    if not raw_text:
        return None

    clean = re.sub(r"\s+", " ", raw_text).strip()
    clean = re.sub(r"^(?:Last Updated|Updated|Date|अंतिम अद्यतन|दिनांक)\s*[:\-]?\s*", "", clean, flags=re.I).strip()
    ...
    # Pattern 3: Hindi DD Month YYYY (e.g. 15 अगस्त 2024)
...
class IndiaGovHTMLStreamingParser(HTMLParser):
    """
    Streaming HTML parser for National Portal of India (india.gov.in / knowindia.india.gov.in).
    """
    ...
    def handle_starttag(self, tag: str, attrs: list):
        ...
        elif tag_lower == "h1" or "portal-title" in class_tokens:
            self._in_h1 = True
            ...
            if self._current_a_href:
                ...
                if ".pdf" in href.lower():
                    ...
                    self.pdf_links.append({
                        "url": full_pdf_url,
                        "title": link_text or "India Portal PDF",
...
class IndiaGovParser(BaseParser):
    """
    Dedicated parser for National Portal of India (india.gov.in / knowindia.india.gov.in).
    Extracts constitutional summaries, governance frameworks, national symbols, directory of schemes,
    ministry profiles, and official PDF documents.
    """
    ...
    def parse(self, response: FetchResponse) -> ParsedContent:
        ...
        streaming_parser = IndiaGovHTMLStreamingParser(base_url=response.url)
        try:
        ...
        title = re.sub(r"\s*-\s*National Portal of India\s*$", "", title, flags=re.I)
        title = re.sub(r"\s*\|\s*National Portal of India\s*$", "", title, flags=re.I)
        title = re.sub(r"\s*-\s*भारत का राष्ट्रीय पोर्टल\s*$", "", title, flags=re.I)
        title = re.sub(r"\s+", " ", title).strip()

        # 2. Extract Category / Sector
        category = None
        cat_match = re.search(r"(?:Category|Sector|श्रेणी)\s*[:\-]\s*([A-Za-z\s&,\(\)]+)", raw_html, re.I)
        if cat_match:
        ...
        published_at = None
        date_patterns = [
            r"(?:Last Updated|Updated|Date|अंतिम अद्यतन|दिनांक)\s*[:\-]?\s*([A-Za-z]{3,9}\s+\d{1,2}[,\s]+\d{4})",
            r"(?:Last Updated|Updated|Date|अंतिम अद्यतन|दिनांक)\s*[:\-]?\s*(\d{1,2}\s+[A-Za-z]{3,9}[,\s]+\d{4})",
            r"(?:Last Updated|Updated|Date|अंतिम अद्यतन|दिनांक)\s*[:\-]?\s*(\d{1,2}\s+[\u0900-\u097F]+\s+\d{4})",
            r"(\d{1,2}[/.-]\d{1,2}[/.-]\d{4})",
        ...
        for dp in date_patterns:
            ...
            if dm:
                parsed_dt = parse_indiagov_date(dm.group(1))
                if parsed_dt:
        ...
        for line in body.splitlines():
            ...
            if not line_str:
                continue
            if "National Informatics Centre" in line_str or "Portal Content Owned" in line_str:
                continue
        ...
        if not title:
            title = "National Portal of India Document"
        ...
        return ParsedContent(
            ...
            metadata=metadata,
            extraction_method="INDIAGOV_ADAPTER",
        )
