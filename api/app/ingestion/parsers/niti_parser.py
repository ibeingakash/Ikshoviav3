...
class NITIHTMLStreamingParser(HTMLParser):
    """
    Streaming HTML parser for NITI Aayog policy reports, flagship indices,
    working papers, initiative overviews, and policy briefings.
    """
            ...
            if self._current_a_href:
                ...
                if ".pdf" in href.lower():
                    ...
                    self.pdf_links.append({
                        "url": full_pdf_url,
                        "title": link_text or "NITI Aayog PDF Report",
...
class NITIParser(BaseParser):
    """
    Dedicated parser for NITI Aayog (National Institution for Transforming India) web pages.
    Extracts policy reports, indices (SDG India Index, Multidimensional Poverty Index,
    State Energy & Climate Index), flagship initiatives, verticals, and PDF reports.
    """
    ...
    def parse(self, response: FetchResponse) -> ParsedContent:
        ...
        streaming_parser = NITIHTMLStreamingParser(base_url=response.url)
        try:
        ...
        title = re.sub(r"\s*-\s*NITI Aayog\s*$", "", title, flags=re.I)
        title = re.sub(r"\s*\|\s*National Institution for Transforming India\s*$", "", title, flags=re.I)
        title = re.sub(r"\s+", " ", title).strip()

        # 2. Extract Vertical / Sector
        vertical = None
        v_match = re.search(r"(?:Vertical|Sector|Division)\s*[:\-]\s*([A-Za-z\s&,\(\)]+)", raw_html, re.I)
        if v_match:
            vertical = v_match.group(1).strip()
        ...
        for dp in date_patterns:
            ...
            if dm:
                parsed_dt = parse_niti_date(dm.group(1))
                if parsed_dt:
                    published_at = parsed_dt
                    break

        # 4. Sanitize Body Text
        ...
        for line in body.splitlines():
            ...
            if not line_str:
                continue
            if "NITI Aayog, Government of India" in line_str or "Designed by NIC" in line_str:
                continue
        ...
        if not title:
            title = "NITI Aayog Policy Report"
        ...
        return ParsedContent(
            ...
            metadata=metadata,
            extraction_method="NITI_ADAPTER",
        )
